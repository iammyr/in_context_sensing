...
class SensingContext extends AbstractResource{
	...
	function __construct ($id)
	{
		...
		$this->triples = '<'.$this->ontology_ns_abbr .':SensingContext rdf:about="'.$this->uri.'" />';
		//		echo "after spo";
	}
...
<rdf:RDF ';
	...
	public function setVersion($version){
		if (isset($version) && trim($version)){
			...
			<'.	$this->dcterms_abbr .':hasVersion rdf:resource="'.
	...
	public function setLocation($locuri){
		if (isset($locuri) && trim($locuri)){
			...
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.	$this->dul_abbr .':hasLocation rdf:resource="'.
			$locuri.'" />
			</rdf:Description>';
		}
	}
	public function setCreator($creator){
		if (isset($creator) && trim($creator)){
			...
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.	$this->dc_abbr .':creator';
			$pos = strpos($creator,"http://");

			if($pos === false) {
				$this->triples .= '>'.$creator.'</'.$this->dc_abbr.':creator>
				</rdf:Description>';
			}else{
	...
	public function setDescription($desc){
		if (isset($desc) && trim($desc)){
			...
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.$this->dc_abbr .':description>'.$desc.'</'.$this->dc_abbr .':description>
			</rdf:Description>';
		}
	}
	...
	public function setTitle($title){
		if (isset($title) && trim($title)){
			$this->title = $title;
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.$this->dc_abbr .":title>".$title.'</'.$this->dc_abbr .":title>
			</rdf:Description>";
		}
	}
	...
	public function setStatus($status){
		if (isset($status) && trim($status)){
			...
			if (strcmp($status, $this->status_live == 0)){
				$this->status_uri=$this->ontology_ns_abbr .':LiveContext';
			}else{
				$this->status_uri=$this->ontology_ns_abbr .':FrozenContext';
			}
	...
	public function setWebsite($website){
		if (isset($website) && trim($website)){
			...
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.	$this->foaf_abbr .':homepage>'.$website.'</'.$this->foaf_abbr .':homepage>
			</rdf:Description>';
		}
	}
	...
	public function setLastModified($updated){
		if (isset($updated) && trim($updated)){
			$this->when = $updated;
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.	$this->dcterms_abbr .":modified>".$updated.'</'.$this->dcterms_abbr .':modified>
			</rdf:Description>';
		}
	}
	...
	public function setTags($tags){
		if (isset($tags)){
			foreach ($tags as $tag){
				...
				$this->triples .= '<'.$this->nepomukao_abbr .':Tag rdf:about="'
				.$tag_uri.'" />';

				$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.$this->nepomukao_abbr.':hasTag rdf:resource="'.$tag_uri.'" />
	</rdf:Description>';
			}
		}
	}
	...
	public function setPlace($place_uri){
		if (isset($place_uri) && trim($place)){
			//			$p = new Place($place);
			$this->triples .= '<rdf:Description rdf:about="'.$this->uri.'">
			<'.$this->dul_abbr .':hasLocation rdf:resource="'.$place_uri.'" />
	</rdf:Description>';
