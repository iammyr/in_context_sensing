...
function getAdditionalNodeTriples($sensor_id_uri, $model_uri, $manual_uri, $stimulus_uri, $capabilities, $observed_property_uri, $conditions){
	...
	$retarr = getCapabilitiesTriple($retarr, $capabilities, $sensor_id_uri, $conditions, $model_uri);

	return $retarr;
}


function getAdditionalOwnerTriples($sensor_id_uri, $sensor_owner, $sensor_publisher, $sensor_location, $foi, $platform_uri, $history_uri, $foi_name){
	$retarr = array();
	...
	$retarr = getFOITriple($retarr, $sensor_id_uri, $foi, $foi_name);
...
function getManualTriple($arr, $manual_uri, $sensor_id_uri){
	//	$retarr = array();
	if ($manual_uri){
		$arr += array($manual_uri => array(
			AbstractResource::$rdf.'type'
			=> array(array('value' => AbstractResource::$eprints.'Document', 'type' => 'uri'))
		));
		if (isset($arr[$sensor_id_uri])&& $arr[$sensor_id_uri]){
			$arr[$sensor_id_uri] += array(
			AbstractResource::$eprints.'hasDocument'
...
function getCapabilitiesTriple($retarr, $capabilities, $sensor_id_uri, $conditions, $model_uri){
	if (count($capabilities) > 0 && $capabilities[0]){
		...
		foreach ($capabilities as $capab) {
			...
			$retarr = getRestrictionTriple($retarr, $capab_class_uri, $model_uri, false, $prop_uri, $uom_uri, $input['max'], $input['min']);
			...
			if (isset($conditions[$cond_count]) && count($conditions[$cond_count]) > 0 && $conditions[$cond_count] ){
				foreach ($conditions[$cond_count] as $cond){
					...
					$retarr = getRestrictionTriple($retarr, $capab_class_uri, $model_uri, true, $cond_prop_uri, $cond_uom_uri, $cond_input['max'], $cond_input['min']);
				}
				$cond_count++;
			}
		}
...
function getRestrictionTriple($retarr, $capab_class_uri, $model_uri, $isCondition, $property, $uom, $max, $min){
...
if ($model_uri){
			$bnode = 'bnode'.(AbstractResource::$bnode_count++);
			if (isset($retarr[$model_uri]) && $retarr[$model_uri]){
				$retarr[$model_uri] += array(
				AbstractResource::$rdfs.'subClassOf'
				=>
				array(array('value'  => $bnode, 'type' => 'bnode'))
				);
			}else{
				$retarr += array(
				$model_uri
...
function getFOITriple($arr, $sensor_id_uri, $foi, $foi_name){
	if ($foi_name){
		if (isset($arr[$sensor_id_uri])&& $arr[$sensor_id_uri]){
			$arr[$sensor_id_uri] += array(
			AbstractResource::$ssn.'featureOfInterest'
			=>
			array(array('value'  => $foi_name, 'type' => 'uri')));
		}else{
			$arr += array(
			$sensor_id_uri
			=>
		...
		$arr += array($foi_name => array(
		AbstractResource::$rdf.'type' => array(array('value' => $foi, 'type' => 'uri'))
		));
	}
	return $arr;
}
...
function getPlatformTriple($arr, $sensor_id_uri, $platform_uri){
	if ($platform_uri){
		if (isset($arr[$sensor_id_uri])&& $arr[$sensor_id_uri]){
			$arr[$sensor_id_uri]  += array(
			AbstractResource::$ssn.'onPlatform'
			=>
