...
function createGeonamesQuery($loc){
	$format = 'json';
	$username='iammyr';
	$query = "http://api.geonames.org/search?q=".urlencode($loc)."&maxRows=10&type=".$format
	."&username=".$username;
	return $query;
}
function locationFinder($location){
	$uri = '';
	$requestURL = createGeonamesQuery($location);
	$response = request($requestURL);
//	echo $response;
	$responseArray = json_decode($response,	true);
	if ($responseArray && $responseArray['geonames'] && $responseArray['geonames'][0]
	&&  $responseArray['geonames'][0]['geonameId']){
		$results = $responseArray['geonames'];
		$geoid = $responseArray['geonames'][0]['geonameId'];
		$uri = "http://sws.geonames.org/".$geoid;	
	}
	if (!$uri){
		$uri = conceptFinder('location', $location);
	}
	return $uri;
}
...
//		if ($responseArray && $responseArray['results'] && 

//$responseArray['results']['bindings']

//		&& $responseArray['results']['bindings'][0] && 

//$responseArray['results']['bindings'][0]['subj']
...
//		}
	...
	if (!$uri || $uri === NULL || !isset($uri) || strcmp($uri, 'null')===0){
