...
abstract class AbstractResource {
	...
	public static $eprints = "http://www.eprints.org/ontology/#ep:";
	public static $eprints_abbr = "eprints";
...
require_once 'ConceptHandler.php';
function getValue($resource, $id, $foaf=false, $location=false, $local=false){
	$ret = $id;
	if (!$local){
		if (isset($id) && $id){
			if (!startsWith($id, "http")){
				if ($location){
					$ret = locationFinder($id);
				}else{
					if (!$foaf){
		...
		$ret = conceptCleaner($ret);
		$ret = AbstractResource::$resource_base.$resource.'/'.trim($ret);
	}
	return $ret;
}
